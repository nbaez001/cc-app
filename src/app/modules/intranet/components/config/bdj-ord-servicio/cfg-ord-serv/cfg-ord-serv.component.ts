...
import { DetalleOrdenServicio } from 'src/app/model/config/detalle-orden-servicio.model';
import { AfectacionPresOS } from 'src/app/model/config/afectacion-pres-os.model';
...
export class CfgOrdServComponent implements OnInit {
  ...
  guardarDetalleOC(): void {
    if (this.formularioGrp2.valid) {
      ...
      this.validationService.setAsUntoched(this.formularioGrp2, this.formErrors2);
    } else {
  ...
  guardarAfectacionPres(): void {
    if (this.formularioGrp3.valid) {
      ...
      this.validationService.setAsUntoched(this.formularioGrp3, this.formErrors3);
    } else {
